 ...
 * 解得：x=(tomatoSlices-2*cheeseSlices)/2;  y=(4*cheeseSlices-tomatoSlices)/2
 */
public class 不浪费原料的汉堡制作方案 {
    ...
    public static List<Integer> numOfBurgers(int tomatoSlices, int cheeseSlices) {
    	...
    	if(x < 0 || ((tomatoSlices-2*cheeseSlices)-(int)(tomatoSlices-2*cheeseSlices)!=0)){
    		return list;
    	}
    	int y = (4*cheeseSlices-tomatoSlices)/2;
    	if(y < 0 || ((4*cheeseSlices-tomatoSlices)-(int)(4*cheeseSlices-tomatoSlices)!=0)){
    		return list;
    	}
    	list.add(x);
    	list.add(y);
